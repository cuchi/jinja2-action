...
class Context:
    ...
    def load_from_input(self):
        for variable in self._environ.get('INPUT_VARIABLES', '').split('\n'):
    ...
    def load_from_data_file(self):
        data_file = self._environ.get('INPUT_DATA_FILE')
        if data_file:
            format = self._environ.get('INPUT_DATA_FORMAT', self._guess_format(data_file))
